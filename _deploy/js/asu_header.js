...
ASUHeader.alterLoginHref = function(url) {
    ...
    if (ASUHeader.signin_callback_url == '') {
    	ASUHeader.signin_callback_url = window.location.toString();
			/* Depricated default_iframe header ID support for 4.0 and 4.1 because browsers don't allow us to run window.parent.location.toString(); statement within iframe.
    ...
    url = url.replace('/login', '/login?callapp=' + encodeURIComponent(ASUHeader.signin_callback_url));
...
//Menu and search toggle (Thanks, Ariel Gold!)
window.onload = function () {
document.getElementById("asu_mobile_button").setAttribute("class", "asushow");
document.getElementById("asu_mobile_menu").setAttribute("class", "toggle_off");
// document.getElementById("asu_mobile_menu").className += " menu-transition";
}
//Menu and search toggle (Thanks, Ariel Gold!)
function toggleASU() {
	var el = document.getElementById('asu_mobile_menu');
	if ( el.className == 'toggle_off' ) {
		el.className = 'toggle_on';
	}
	else {
		el.className = 'toggle_off';		
	}
}
